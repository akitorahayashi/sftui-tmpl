...
**重要:** このテンプレートを新しいプロジェクトで使用する際は、まず最初にルート直下にある `TemplateApp` ディレクトリの名前を、あなたのプロジェクト名に変更してください（例: `MyAwesomeApp`）。同様に `TemplateAppTests` や `TemplateAppUITests` ディレクトリも、新しいテストターゲット名に合わせて変更することを推奨します。これらの変更は、後述する多くの設定ファイル中のパス記述に影響します。最初にディレクトリ名を変更することで、後続のカスタマイズ作業がスムーズになります。ディレクトリ名変更後、Xcodeプロジェクトを一度開いて、関連するファイル名やクラス名（例: `TemplateApp` 構造体、`TemplateAppTests` クラスなど）が正しくリファクタリングされているか確認し、必要であれば手動で修正してください。
